...
function exportReport(messages: UIMessage[]): string {
  const html = `
    ...
    <head>
      <meta charset="utf-8">
      <title>SentinelBrain Incident Report</title>
      <style>
        ...
        h2 { color: #1e293b; margin-top: 30px; background: #f8fafc; padding: 10px; border-radius: 6px; font-size: 1.1em; }
        .user { color: #4f46e5; font-weight: bold; }
        .assistant { color: #7c3aed; font-weight: bold; }
        .timestamp { font-size: 0.8em; color: #64748b; margin-bottom: 20px; }
        pre { background: #0f172a; color: #e2e8f0; padding: 15px; border-radius: 8px; overflow-x: auto; font-size: 0.85em; }
        .tool-header { font-weight: bold; color: #6366f1; margin-top: 15px; font-size: 0.85em; text-transform: uppercase; }
        .footer { margin-top: 50px; font-size: 0.8em; color: #94a3b8; text-align: center; border-top: 1px solid #e2e8f0; padding-top: 20px; }
        .message { margin-bottom: 30px; }
      </style>
    </head>
    <body>
      <h1>SentinelBrain Incident Report</h1>
      <div class="timestamp">Generated on ${new Date().toLocaleString()}</div>
      
      ${messages.map(msg => `
        <div class="message">
          <h2 class="${msg.role}">${msg.role === 'user' ? '👤 Engineer' : '🤖 SentinelBrain'}</h2>
          ${msg.parts.map(part => {
            if (part.type === 'text') {
              return `<p>${part.text.replace(/\n/g, '<br>')}</p>`;
            } else {
              ...
              if (toolName && partAny.state === 'output-available') {
                return `
                  <div class="tool-header">Tool Execution: ${toolName}</div>
                  <pre>${JSON.stringify(partAny.output, null, 2)}</pre>
                `;
              }
              return '';
            }
      ...
      <div class="footer">
        Generated by SentinelBrain AI-Native DevOps Copilot<br>
        &copy; 2026 SentinelBrain Inc.
      </div>
    </body>
    </html>
  `;
...
export default function SentinelBrainPage() {
  ...
  const handleExport = useCallback(() => {
    const htmlContent = exportReport(messages);
    const blob = new Blob([htmlContent], { type: "application/msword" });
    const url = URL.createObjectURL(blob);
    const link = document.createElement("a");
    link.href = url;
    link.download = `incident-report-${new Date().getTime()}.doc`;
